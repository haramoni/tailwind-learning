...
const fileItem = tv({
  slots: {
    ...
    icon: "rounded-full border-4 border-violet-100 bg-violet-200 p-2 text-violet-600 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-500 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-500",
    deleteButton: "",
  },
  ...
  variants: {
    state: {
      ...
      complete: {
        container:
          "border-violet-500 dark:border-violet-300/30",
      },
      error: {
        container:
...
export interface FileItemProps
  extends VariantProps<typeof fileItem> {
  name: string;
  size: number;
}
...
export function FileItem({
  ...
  const { container, icon, deleteButton } =
    fileItem({ state });
